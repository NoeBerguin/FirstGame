...
void UMultiplayerSessionsSubsystem::CreateSession(int32 NumPublicConnections, FString MatchType)
{
    ...
    LastSessionSettings->Set(FName("GameVersion"), FString("1.0.0"));

    // ✅ Récupérer le pseudo Steam du joueur
    ...
    if (OnlineSubsystem) {
        ...
        if (IdentityInterface.IsValid()) {
            ...
            if (LocalPlayer) {
                ...
                if (PlayerId.IsValid()) {
                    PlayerName = IdentityInterface->GetPlayerNickname(*PlayerId);
                }
            }
        }
    }
    LastSessionSettings->Set(FName("SessionOwner"), PlayerName, EOnlineDataAdvertisementType::ViaOnlineServiceAndPing);
